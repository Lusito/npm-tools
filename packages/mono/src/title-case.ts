// copied from https://github.com/blakeembrey/change-case as they don't support commonjs anymore
const TOKENS = /(\S+)|(.)/g;
...
export const TITLE_TERMINATORS = new Set([...SENTENCE_TERMINATORS, ":", '"', "'", "”"]);
...
export function titleCase(input: string, options: Options | string[] | string = {}) {
    const {
        locale = undefined,
        sentenceCase = false,
...
function upperAt(input: string, index: number, locale: string | string[] | undefined) {
